...
class MASShortcutValidator: NSObject {
	
	static var once = UnsafeMutablePointer<dispatch_once_t>.alloc(1)
	static var sharedInstance:MASShortcutValidator!
	
	class func sharedValidator() -> MASShortcutValidator {
		
		dispatch_once(
			self.once,
			{ self.sharedInstance = MASShortcutValidator() }
		)
	...
	func isShortcutValid( shortcut:MASShortcut ) -> Bool {
	...
	@objc func isShortcut(
		...
		for menuItem in menu.itemArray as! [NSMenuItem] {
			...
			var equalFlags = MASPickCocoaModifiers( UInt(menuItem.keyEquivalentModifierMask) ) == flags
			...
			if equalHotkeyLowercase && menuItem.keyEquivalent != keyEquivalent {
				...
				equalFlags = MASPickCocoaModifiers(
